...
public class AvgPricePanel extends AsianoptionsPanel{
...
//						double noRiskRate = Double.parseDouble(noRiskRateField.getText());
						...
						try {
							PurchasePrice = service.getSubtypeAveragePricePrice(eora, upordown, executePrice, deadline, "131250131");
				...
				public void mouseClicked(MouseEvent e) {
					...
					if(dealNumField.getText().equals("")||BorA.getSelection()==null){
						...
						if(timer.getText().equals("(请在0秒内完成操作)")){
							...
							//调用交易接口
							option =  new Option("亚式a期权", "平均价格期权", eora, upordown);
